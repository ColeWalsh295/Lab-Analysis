...
plot.unweight <- ggplot(df.density %>%
         filter(variable == 'density'), aes(x = as.factor(week), y = value, group = method.lab, color = as.factor(method), shape = as.factor(lab))) +
  geom_point(size = 4) +
...
plot.weight <- ggplot(df.density %>%
                        ...
                        mutate(method = ifelse(method == 1, 'one', 'two')), aes(x = as.factor(week), y = value, group = method.lab, color = as.factor(method), shape = as.factor(lab))) +
  geom_point(size = 4) +
  geom_line(stat = 'identity', size = 1) +
